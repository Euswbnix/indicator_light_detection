...
Dense small-light case: two adjacent small-light boxes may overlap heavily (IoU>0.5) but have
different centers. Standard NMS would merge them. DIoU subtracts a "center-distance penalty" from
IoU, so boxes with well-separated centers are kept even when they overlap -- exactly what's needed
to stop a row of status-bar small lights from being merged into one box.
"""
import numpy as np
